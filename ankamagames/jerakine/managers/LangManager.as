package com.ankamagames.jerakine.managers
{
   ...
   public class LangManager
   {
      ...
      private static var _self:com.ankamagames.jerakine.managers.LangManager;
      
      protected static const _log:Logger = Log.getLogger(getQualifiedClassName(com.ankamagames.jerakine.managers.LangManager));
      ...
      private var _parseReference:Array;
      
      private var _fontManager:com.ankamagames.jerakine.managers.FontManager;
      
      public function LangManager()
      {
         this._parseReference = new Array();
         super();
         if(_self != null)
         {
      ...
      public static function getInstance() : com.ankamagames.jerakine.managers.LangManager
      {
         if(_self == null)
         {
            _self = new com.ankamagames.jerakine.managers.LangManager();
         }
         return _self;
      }
         ...
         if(sTxt != null && sTxt.indexOf("[") != -1)
         {
            ...
            for(i = 0; i < aKey.length; i++)
            {
               ...
               if(sNewVal == null)
               {
                  ...
                  if(I18nProxy.containKey(sKey))
                  {
                     sNewVal = I18n.getText(I18nProxy.getKeyId(sKey));
                  }
                  else
                  {
                     if(sKey.charAt(0) == "~")
      ...
      private function onXmlLoadComplete(e:ResourceLoadedEvent) : void
      {
         ...
         if(Boolean(metaData.clearFile[e.uri.fileName]) || Boolean(metaData.clearAllFile) || Boolean(metaData.loadAllFile))
         {
            if(Boolean(metaData.clearFile[e.uri.fileName]) || Boolean(metaData.clearAllFile))
            {
               this.clear(sCat);
            }
      ...
      private function onZipFileComplete(e:ResourceLoadedEvent) : void
      {
         ...
         for(i = 0; i < aFileList.length; i++)
         {
            ...
            if(Boolean(metaData.clearFile[entry.name]) || Boolean(metaData.clearAllFile) || Boolean(metaData.loadAllFile))
            {
               if(Boolean(metaData.clearFile[entry.name]) || Boolean(metaData.clearAllFile))
               {
                  this.clear(sCat);
               }
               if(Boolean(metaData.clearFile[entry.name]) || Boolean(metaData.loadAllFile))
               {

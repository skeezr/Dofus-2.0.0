package com.ankamagames.jerakine.data
{
   ...
   public class AbstractDataManager
   {
      ...
      private var _cacheSO:ICache;
      
      private var _cacheKey:ICache;
      ...
      public function getObject(key:uint) : Object
      {
         var v:* = undefined;
         var foo:* = undefined;
         var so:DestroyableSharedObject = null;
         ...
         if(this._cacheSO.contains(soName))
         {
            ...
            v = DestroyableSharedObject(this._cacheSO.peek(soName)).data[DATA_KEY][key];
            this._cacheKey.store(realKey,v);
            return v;
         }
         so = DestroyableSharedObject.getLocal(soName);
      ...
      public function getObjects() : Array
      {
         ...
         for each(fileNum in fileList)
         {
            ...
            if(this._cacheSO.contains(soName))
            {
               data = data.concat(DestroyableSharedObject(this._cacheSO.peek(soName)).data[DATA_KEY]);
            }
            else
            {
               so = DestroyableSharedObject.getLocal(soName);

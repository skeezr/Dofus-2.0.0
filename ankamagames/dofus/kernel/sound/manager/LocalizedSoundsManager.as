      ...
      public function playLocalizedSounds() : void
      {
         ...
         for each(lms in this._localizedSounds)
         {
            ...
            if(SoundManager.getInstance().manager is ClassicSoundManager)
            {
               ...
               localizedSound.bus = busId;
               (localizedSound as LocalizedSound).saturationRange = lms.saturationRange;
               (localizedSound as LocalizedSound).silenceMin = lms.silenceMin;
               (localizedSound as LocalizedSound).silenceMax = lms.silenceMax;
               ...
               SoundManager.getInstance().manager.playSound(localizedSound,localizedSound.silenceMin == 0 && localizedSound.silenceMax == 0);
            }
            if(SoundManager.getInstance().manager is RegSoundManager)
            {
               ...
               (localizedSound as SoundDofus).silenceMin = lms.silenceMin;
               (localizedSound as SoundDofus).silenceMax = lms.silenceMax;
               ...
               localizedSound.play(localizedSound.silenceMin == 0 && localizedSound.silenceMax == 0);
            }

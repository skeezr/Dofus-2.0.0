package com.ankamagames.berilia.managers
{
   ...
   public class CssManager
   {
      ...
      protected static const _log:Logger = Log.getLogger(getQualifiedClassName(com.ankamagames.berilia.managers.CssManager));
      
      private static const CSS_ARRAY_KEY:String = "cssFilesContents";
      
      private static var _self:com.ankamagames.berilia.managers.CssManager;
      ...
      public static function getInstance() : com.ankamagames.berilia.managers.CssManager
      {
         if(!_self)
         {
            _self = new com.ankamagames.berilia.managers.CssManager();
         }
         return _self;
      }
         ...
         for(url in this._aMultiWaiting)
         {
            if(this._aMultiWaiting[url])
            {
               ...
               for(i = 0; i < this._aMultiWaiting[url].length; i++)
               {
                  ...
                  ok = Boolean(ok) && this._aMultiWaiting[url][i] === true;
               }
               if(ok)
               {
      ...
      {
         _log.fatal("Cannot find " + e.uri + ", abord css parsing process.");
         this._aLoadingFile[e.uri.uri] = false;

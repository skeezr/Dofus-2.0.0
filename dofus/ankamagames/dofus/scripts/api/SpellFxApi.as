package com.ankamagames.dofus.scripts.api
{
   ...
   public class SpellFxApi extends FxApi
   {
      ...
      public static function GetSpellParam(spell:CastingSpell, name:String) : Number
      {
         var r:Number = spell.spell.getParamByName(name);
         return !!isNaN(r)?Number(0):Number(r);
      }
      
      public static function HasSpellParam(spell:CastingSpell, name:String) : Boolean
      {
         return !isNaN(spell.spell.getParamByName(name));
      }
      ...
      public static function CreateExplosionEntity(runner:SpellFxRunner, gfxId:uint, startColors:String, particleCount:uint, levelChange:Boolean, subExplo:Boolean) : ExplosionEntity
      {
         var tmp:Array = null;
         var i:uint = 0;
         var spellRank:int = 0;
         ...
         if(levelChange)
         {
            ...
            if(spellRank != -1)
            {
               particleCount = particleCount * runner.castingSpell.spellRank.spell.spellLevels.length / 10 + particleCount * spellRank / 10;
            }
         }
         return new ExplosionEntity(uri,tmp,particleCount,subExplo);
      }
   }
}

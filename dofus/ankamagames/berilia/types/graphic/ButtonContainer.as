package com.ankamagames.berilia.types.graphic
{
   ...
   public class ButtonContainer extends StateContainer implements IRadioItem, FinalizableUIComponent, IDragAndDropHandler
   {
      ...
      protected var _soundId:String = "-1";
      ...
      {
         this._checkbox = b;
         this._radioMode = !b && Boolean(this._radioMode);
      }
      ...
      {
         this._radioMode = b;
         this._checkbox = !b && Boolean(this._checkbox);
      }
      ...
      public function finalize() : void
      {
         ...
         if(this._radioGroup)
         {
            rg = getUi().addRadioGroup(this._radioGroup);
            rg.addItem(this);
         }
         if(this._selected)
         {
      ...
      override public function process(msg:Message) : Boolean
      {
         ...
         if(!this._disabled)
         {
            switch(true)
            {
               ...
               case msg is MouseOverMessage:
                  ...
                  {
                     if(Boolean(this._playRollOverSound) && !this.isMute)
                     {
         ...
         {
            this.state = tmpState;
            if(Boolean(this._radioGroup) && Boolean(this._selected))
            {
               rg = getUi().getRadioGroup(this._radioGroup);
               if(rg)
               {
         ...
         if(this._sLinkedTo)
         {
            getUi().getElement(this._sLinkedTo).process(msg);
         }
         return false;
      }
   }

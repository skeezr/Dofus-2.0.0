   ...
   public class TabSet extends GraphicContainer implements FinalizableUIComponent
   {
      ...
      public function set length(i:int) : void
      {
         ...
         if(this._btnPlus)
         {
            ...
            if(this._nNbTabsRequired < this._nNbTabs)
            {
               _log.error("Cannot remove random tabs from tabset.");
            }
         }
      }
      ...
      private function removeTab() : void
      {
         ...
         _log.warn("removeTab " + this._nSelected + "  -> " + this._nPreviousSelected);
         if(this._nNbTabs > 1)
         {
            indexAlmostRemoved = this._nSelected;
            this.selectedTab = this._nPreviousSelected;
            this._nPreviousSelected = -1;
      ...
      private function replaceTab() : void
      {
         ...
         _log.warn("replaceTab " + this._nSelected + "  -> " + index);
         if(this._nSelected == -1)
         {
      ...
      private function switchToEdition(value:Boolean) : void
      {
         if(value)
         {
            this._bNameEdition = true;
            this._aInputs[this._nSelected].disabled = false;
            this._aInputs[this._nSelected].text = this._aLbls[this._nSelected].text;
            this._aInputs[this._nSelected].visible = true;
            this._aInputs[this._nSelected].focus();
            this._aInputs[this._nSelected].textfield.setSelection(0,this._aInputs[this._nSelected].text.length);
            this._aLbls[this._nSelected].visible = false;
         }
         else
         {
            this._bNameEdition = false;
            this._aInputs[this._nSelected].visible = false;
            this._aInputs[this._nSelected].disabled = true;
            this._aLbls[this._nSelected].visible = true;
         }
      }
            ...
            case msg is MouseClickMessage:
               ...
               switch(mcm.target.name)
               {
                  case this._btnPlus.name:
                     ...
                     {
                        this.addTab();
                        _log.warn("addTab " + this._nSelected + "  -> " + (this._nNbTabs - 1));
                  ...
                  default:
                     for(i in this._aCtrs)
                     {
                        ...
                        {
                           _log.warn("clicTab " + this._nSelected + "  -> " + i);
                           this.selectedTab = i;
                        }
                     }
